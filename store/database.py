...
class Store(object):
    ...
    @db_session
    def search_elems(self, condition, for_update=False, fuzzy=True, debug=False, count=True):
        elems = select(e for e in self.store)
        if condition:
    ...
    @db_session
    def search(self, condition, for_update=False, fuzzy=True, debug=False, count=True, begin=None, end=None):
        elems = select(e for e in self.store)
        if condition:
            for key, value in condition.items():
                if isinstance(value, list):
